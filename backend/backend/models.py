...
class Point(models.Model):
    ...
    server_player = models.IntegerField(
        null=True, blank=True, help_text="Player serving (e.g., 1 or 2)"
    )
    team1_points_at_start = models.CharField(
        max_length=10,
        null=True,
        blank=True,
        help_text="Team 1 points (0, 15, 30, 40, AD) at start",
    )
    team2_points_at_start = models.CharField(
        max_length=10,
        null=True,
        blank=True,
        help_text="Team 2 points (0, 15, 30, 40, AD) at start",
    )
    team1_games_at_start = models.IntegerField(
        null=True, blank=True, help_text="Team 1 games won at start of point"
    )
    team2_games_at_start = models.IntegerField(
        null=True, blank=True, help_text="Team 2 games won at start of point"
    )
    set_number_at_start = models.IntegerField(
        null=True, blank=True, help_text="Current set number at start of point"
    )

    # === Context at Point End ===
    # Note: These might reflect the score *after* the point outcome is decided
    team1_points_at_end = models.CharField(
        max_length=10,
        null=True,
        blank=True,
        help_text="Team 1 points (0, 15, 30, 40, AD) at end",
    )
    team2_points_at_end = models.CharField(
        max_length=10,
        null=True,
        blank=True,
        help_text="Team 2 points (0, 15, 30, 40, AD) at end",
    )
    team1_games_at_end = models.IntegerField(
        null=True, blank=True, help_text="Team 1 games won at end of point"
    )
    team2_games_at_end = models.IntegerField(
        null=True, blank=True, help_text="Team 2 games won at end of point"
    )
    set_number_at_end = models.IntegerField(
        null=True, blank=True, help_text="Current set number at end of point"
    )
    ...
    def __str__(self):
        start_str = (
            self.recorded_start_time.strftime("%Y-%m-%d %H:%M:%S")
...
class Coordinate(models.Model):
    ...
    point = models.ForeignKey(
        Point, related_name="coordinates", on_delete=models.CASCADE
    )
    # Timestamp relative to the start of *this specific point's simulation/recording*
    relative_time_ms = models.PositiveIntegerField(
        db_index=True, help_text="Milliseconds since point start"
    )  # Added index
